# All-Day Build Contract: agentkit-cli v1.29.0 release completion
...
6. Never modify files outside `/Users/mordecai/repos/agentkit-cli-v1.29.0-flagship-self-advance`.
...
- [ ] Reconcile any stale prose before proceeding.
...
- [ ] Create or verify annotated tag `v1.29.0` on the intended release commit.
...
Publish `agentkit-cli==1.29.0` to PyPI and verify both project and version endpoints plus artifacts.
...
- [ ] Publish only the `1.29.0` wheel and sdist.
- [ ] Verify PyPI project JSON and version JSON both report `1.29.0` live.
...
- [ ] Edge cases: tag already exists, branch already pushed, publish auth path, dist contamination from old artifacts, shipped commit vs docs-only chronology head
...
- Before trusting release/status narrative, run `scripts/pre-action-recall.sh release agentkit-cli /Users/mordecai/repos/agentkit-cli-v1.29.0-flagship-self-advance`.
- Run `scripts/check-status-conflicts.sh /Users/mordecai/repos/agentkit-cli-v1.29.0-flagship-self-advance` before final summary.
- Run `scripts/post-agent-hygiene-check.sh /Users/mordecai/repos/agentkit-cli-v1.29.0-flagship-self-advance` before final summary.
- If blocked, write `blocker-report-v1.29.0-release.md` with exact failure, attempts, and recommended next move.
