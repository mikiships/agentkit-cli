# All-Day Build Contract: agentkit-cli v1.7.0 release completion
...
Take the completed `agentkit taskpack` feature in `/Users/mordecai/repos/agentkit-cli-v1.7.0-taskpack-handoff` from truthful local release-ready state to truthful shipped-or-blocked release truth. The feature build is done, validation is green, and the remaining work is the full four-surface release checklist plus chronology reconciliation. This pass must verify the exact repo state, execute release only if every gate passes, and leave every status/report file telling one coherent story.
...
4. Never modify files outside `/Users/mordecai/repos/agentkit-cli-v1.7.0-taskpack-handoff`.
...
8. Read existing release docs, tests, and report surfaces before editing them.
...
Required:
- focused taskpack validation slice
- full supported suite, unless a narrower baseline is explicitly justified and documented in the repo reports

- [ ] Re-run the focused `agentkit taskpack` release slice
...
Push the release branch or current release commit and the annotated tag once validation is green and the repo state is clean.

Required:
- git branch on origin
- annotated `v1.7.0` tag on origin

- [ ] Push the release branch or current release commit to origin
- [ ] Create or verify the annotated `v1.7.0` tag at the tested release commit
...
Required:
- `dist/` artifacts
- PyPI JSON/page proof for `agentkit-cli==1.7.0`

- [ ] Build wheel and sdist from the tested release state
- [ ] Publish `agentkit-cli==1.7.0` via the supported authenticated path
...
- [ ] Run hygiene and conflict checks on the final repo state
- [ ] Leave the repo clean except for intentional, documented artifacts
- [ ] Final summary with exact tests, refs, and registry proof

## 4. Test Requirements

- [ ] Focused taskpack validation slice passes
- [ ] Full test suite passes under the repo's supported environment, or the release report explicitly justifies a narrower supported baseline
...
- [ ] Registry verification confirms both wheel and sdist for `1.7.0`
