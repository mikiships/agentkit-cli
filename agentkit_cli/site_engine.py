...
.repo-delta-grid, .delta-points, .setup-paths {
  margin-top: 1.5rem;
  display: grid;
  gap: 1rem;
}
...
.delta-shell, .delta-point, .setup-path {
  background: var(--surface);
  border: 1px solid var(--border);
  border-radius: 14px;
  padding: 1.25rem;
...
.delta-note, .delta-point p, .setup-path p {
  color: var(--muted);
  font-size: 0.92rem;
  line-height: 1.7;
}
...
@media (max-width: 900px) {
  .hero-proof, .workflow-grid, .feature-grid, .trust-grid, .secondary-grid, .quickstart-shell, .repo-delta-grid, .delta-points, .setup-paths {
...
_NAV = """<header>
  ...
  <nav>
    ...
    <a href="{base_url}#repo-diff">Repo example</a>
...
class SiteEngine:
    ...
    def generate_index(self, site_data: Optional[dict] = None) -> SitePage:
        ...
        <div class="hero">
          ...
          <p class="hero-sub">If you landed here from HN, the pitch is simple: keep one repo-owned source of truth at <code>.agentkit/source.md</code>, project it into AGENTS.md, CLAUDE.md, GEMINI.md, COPILOT.md, AGENT.md, and <code>llms.txt</code>, then score whether the repo is actually ready for agent work.</p>
          <div class="hero-actions">
            <div class="install-block"><span class="prompt">$</span> pip install agentkit-cli</div>
            <a href="#repo-diff" class="nav-cta">See what changes in a repo ↓</a>
          ...
          <div class="hero-proof">
            <div class="proof-item">
              ...
              <strong>Author one file, not six drifting ones.</strong>
              <p>Keep the canonical instructions in <code>.agentkit/source.md</code>, then project the filenames your tools already read.</p>
            </div>
            <div class="proof-item">
              <span class="proof-kicker">Repo diff</span>
              <strong>The first output is a git diff you can review.</strong>
              <p>Run <code>source --init</code> and <code>project --write</code> to add real files to the repo instead of hiding setup in another hosted UI.</p>
            </div>
            <div class="proof-item">
              <span class="proof-kicker">Guardrails</span>
              <strong>Get a fast score, then add gates when the repo is ready.</strong>
              <p>Use <code>quickstart</code> for the first read, <code>score</code> for the repo-owned setup, and <code>gate</code> once you want CI enforcement.</p>
            </div>
            <div class="proof-item">
        ...
        <section class="repo-diff-section" id="repo-diff">
          <div class="container section-frame">
            <div class="section-header">
              <div>
                <h2 class="section-title">What changes in a real repo</h2>
                <p class="section-lead">The first visible output is a repo diff. agentkit does not ask you to trust a hosted dashboard before it changes anything useful in the repo itself.</p>
              </div>
            </div>
            <div class="repo-delta-grid">
  ...
  src/
  tests/</code></pre>
                <p class="delta-note">Normal codebase, but no shared agent instructions. Each tool starts from a different file or from no file at all.</p>
              </div>
              <div class="delta-shell">
                <span class="delta-label">After <code>source --init</code> + <code>project --write</code></span>
                <pre class="code-block"><code>repo/
  .agentkit/source.md
  AGENTS.md
  CLAUDE.md
  ...
  src/
  tests/</code></pre>
                <p class="delta-note">One repo-owned source fans out into the filenames Claude Code, Codex, Gemini, Copilot, and other agent surfaces already look for.</p>
              </div>
            </div>
            <div class="delta-points">
              <div class="delta-point"><h3>Reviewable in git</h3><p>The setup becomes a normal repo change that maintainers can diff, review, and revert.</p></div>
              <div class="delta-point"><h3>Shared across agents</h3><p>You edit the source once instead of hand-syncing separate agent files every time the repo rules change.</p></div>
              <div class="delta-point"><h3>Measurable afterward</h3><p>Run <code>agentkit score</code> or <code>agentkit gate</code> after the files exist so the workflow stays inspectable.</p></div>
            </div>
          </div>
        </section>
        ...
        <section class="quickstart-section" id="quickstart">
          <div class="container section-frame">
            <div class="section-header">
              <div>
                <h2 class="section-title">Install to first value</h2>
                <p class="section-lead">There are two honest entry points: use <code>quickstart</code> if you want a score before editing files, or use the canonical-source flow if you already want repo-owned agent instructions in git.</p>
              </div>
            </div>
            <div class="setup-paths">
              <div class="setup-path">
                <span class="proof-kicker">Path 1</span>
                <h3>Get a score first</h3>
                <p>Best for skeptical first runs. You learn where the repo stands before adopting any workflow changes.</p>
                <pre class="code-block"><code>pip install agentkit-cli
agentkit quickstart .</code></pre>
                <ul class="quickstart-list">
                  <li><strong><code>quickstart</code></strong> runs doctor plus a fast composite score on the current repo.</li>
                  <li><strong>No file writes required.</strong> You get findings and a score before deciding whether to standardize anything.</li>
                </ul>
              </div>
              <div class="setup-path">
                <span class="proof-kicker">Path 2</span>
                <h3>Write the repo-owned setup</h3>
                <p>Best when you already know the repo needs one shared source of instructions across multiple agent surfaces.</p>
                <pre class="code-block"><code>agentkit source --init
agentkit project --write
agentkit score</code></pre>
                <ul class="quickstart-list">
                  <li><strong><code>source --init</code></strong> creates <code>.agentkit/source.md</code> as the file you actually maintain.</li>
                  <li><strong><code>project --write</code></strong> writes the projected agent files next to the code.</li>
                  <li><strong><code>score</code></strong> tells you if the repo is ready for gates and repeatable agent work.</li>
                </ul>
              </div>
            </div>
        ...
        <section class="workflow-section">
          <div class="container section-frame">
            ...
            <div class="workflow-grid">
              ...
              <div class="workflow-card"><span class="step">03</span><h3>Score, gate, and learn</h3><p>Run <code>score</code>, fail CI with <code>gate</code>, benchmark execution quality, and inspect transcripts so the workflow gets better over time.</p></div>
            </div>
          </div>
        </section>
        ...
        <section class="secondary-section" id="use-cases">
          <div class="container">
            <div class="section-header">
              <div>
                <h2 class="section-title">Who should use this now</h2>
                <p class="section-lead">This is most useful once agent work is a repo problem, not just a personal prompt problem. The value starts when the instructions need to live in git and stay consistent across tools or teammates.</p>
              </div>
            </div>
            <div class="secondary-grid">
              <div class="secondary-card">
                <h3>More than one agent touches the repo</h3>
                <p>You already have Claude Code, Codex, Gemini, Copilot, or MCP tools touching the same codebase and you do not want their instructions drifting apart.</p>
              </div>
              <div class="secondary-card">
                <h3>You want repo-owned instructions</h3>
                <p>You want the setup to be a normal change in git, with files maintainers can review, diff, and enforce in CI.</p>
              </div>
              <div class="secondary-card">
                <h3>You want proof before policy</h3>
                <p>You want to score real repos, inspect public examples, and measure readiness before telling a team to standardize on a workflow.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="features-section" id="trust">
          <div class="container">
            ...
            <div class="trust-grid" style="margin-bottom:1rem">
              ...
              <div class="trust-card"><h3>Workflow surface, not a toy</h3><p>The toolkit already spans quickstart, source management, projection, scoring, linting, benchmarking, transcript review, and MCP access across <strong>{frontdoor['packages']} packages</strong>, with <strong>{pypi_month_display}</strong> downloads in the last 30 days alone.</p></div>
            </div>
            <div class="section-header">
              <div>
            ...
            <div class="feature-grid">
              <div class="feature-card"><h3>agentkit-cli</h3><p>The orchestration entry point for quickstart, source, project, score, gate, and guard flows.</p></div>
        ...
        <section class="commands-section">
          <div class="container">
            ...
            <table class="cmd-table">
              ...
              <tbody>
                <tr><td>agentkit quickstart .</td><td>Run the fastest honest first pass: doctor plus a fast composite score on the current repo</td></tr>
        ...
        meta = PageMeta(
            ...
            description="Keep one repo-owned source for AI coding agents, project it into AGENTS.md and other agent files, then score and gate the repo with agentkit-cli.",
