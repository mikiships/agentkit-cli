...
from agentkit_cli.launch import LaunchEngine, LaunchError
...
def launch_command(
    ...
    try:
        ...
        plan = engine.launch(project, target=target) if execute else engine.build(project, target=target)
    except (FileNotFoundError, LaunchError, ValueError) as exc:
    ...
    if output_dir is not None:
        ...
        typer.echo(f"Wrote launch directory: {written}")
    typer.echo(rendered)
