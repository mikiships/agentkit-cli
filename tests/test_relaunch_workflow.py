...
def test_launch_to_observe_to_supervise_to_reconcile_to_resume_to_relaunch_workflow(tmp_path):
    ...
    relaunch_dir = tmp_path / "relaunch"
    relaunch = runner.invoke(app, ["relaunch", str(project), "--output-dir", str(relaunch_dir), "--json"])
    assert relaunch.exit_code == 0, relaunch.output
    payload = json.loads("\n".join(relaunch.output.splitlines()[1:]))
    ...
    assert (relaunch_dir / "lanes" / "lane-02" / "relaunch.md").exists()
    assert (relaunch_dir / "lanes" / "lane-02" / "handoff.md").exists()
