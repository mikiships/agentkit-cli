...
def test_launch_to_observe_to_supervise_to_reconcile_to_resume_workflow(tmp_path):
    ...
    resume = runner.invoke(app, ["resume", str(project), "--output-dir", str(resume_dir), "--json"])
    assert resume.exit_code == 0, resume.output
    payload = json.loads("\n".join(resume.output.splitlines()[1:]))
    ...
    assert (resume_dir / "lanes" / "lane-02" / "resume.md").exists()
