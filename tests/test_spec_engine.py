...
def test_spec_engine_emits_concrete_flagship_next_step_after_shipped_truth_sync(tmp_path):
    ...
    assert spec.primary_recommendation.kind == "flagship-concrete-next-step"
    assert spec.primary_recommendation.contract_seed.title.endswith("spec concrete next step")
    assert any("generic `subsystem-next-step`" in item for item in spec.primary_recommendation.evidence)
