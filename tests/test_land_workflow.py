...
def test_launch_to_observe_to_supervise_to_reconcile_to_resume_to_relaunch_to_closeout_to_land_workflow(tmp_path):
    ...
    closeout = runner.invoke(app, ["closeout", str(project), "--output-dir", str(closeout_dir)])
    ...
    land = runner.invoke(app, ["land", str(project), "--output-dir", str(land_dir), "--json"])
    assert land.exit_code == 0, land.output
    payload = json.loads("\n".join(land.output.splitlines()[1:]))
    ...
    assert payload["landing_order_lane_ids"] == ["lane-01"]
    assert (land_dir / "lanes" / "lane-01" / "packet.md").exists()
    assert (land_dir / "lanes" / "lane-02" / "land.md").exists()
