...
def test_version_is_0_55_0():
    assert 'version = "0.55.0"' in _pyproject()


def test_changelog_has_0_55_0():
    assert "0.55.0" in _changelog()


def test_changelog_entry_at_top():
    lines = _changelog().splitlines()
    # First version heading should be 0.55.0
    for line in lines:
        if line.startswith("## ["):
            assert "0.55.0" in line, f"First version in CHANGELOG is not 0.55.0: {line}"
            break
...
def test_agentkit_version_command():
    """agentkit --version should report 0.55.0."""
    ...
    assert "0.55.0" in combined or result.returncode != 0  # acceptable if version not in --version flag
