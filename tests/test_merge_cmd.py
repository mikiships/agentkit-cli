...
def test_merge_command_writes_packet_directory_and_json(tmp_path):
    ...
    output_dir = tmp_path / "merge-report"
    result = runner.invoke(app, ["merge", str(project), "--output-dir", str(output_dir), "--json"])

    assert result.exit_code == 0, result.output
    payload = json.loads("\n".join(result.output.splitlines()[1:]))
    assert payload["schema_version"] == "agentkit.merge.v1"
    assert payload["merge_now_lane_ids"] == ["lane-01"]
    assert (output_dir / "merge.md").exists()
    assert (output_dir / "merge.json").exists()
