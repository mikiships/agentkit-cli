...
| D4 | ✅ Complete | Reconciled repo and workspace chronology so the shipped tag truth stays separate from the later docs-only branch head on `origin/feat/v1.22.0-spec`. |
...
- Git surface: `origin/feat/v1.22.0-spec` now points to a later docs-only chronology head, annotated tag `v1.22.0` exists on origin, and `v1.22.0^{}` peels to shipped release commit `2c2b89f`
...
- The release branch later advanced to a later docs-only chronology head after release gating notes were committed.
