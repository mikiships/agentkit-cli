# All-Day Build Contract: agentkit-cli v1.3.0 release completion
...
Take the completed `agentkit map` feature in `/Users/mordecai/repos/agentkit-cli-v1.3.0-map` from local release-ready state to truthful shipped-or-blocked release truth. The feature build is done, but the release surfaces are not. This pass must verify the exact repo state, clean up any release-noise drift, execute the four-surface release checklist if all gates pass, and leave every status/report file telling one coherent story.
...
4. Never modify files outside `/Users/mordecai/repos/agentkit-cli-v1.3.0-map`.
...
Required:
- `BUILD-REPORT.md`
- `BUILD-REPORT-v1.3.0.md`
- `progress-log.md`
- `CHANGELOG.md`
- repo status / tag state

- [ ] Run release recall plus contradiction checks before trusting any narrative
- [ ] Reconcile dirty repo state, especially `.agentkit-last-run.json`, contract-file noise, and any version/report drift
...
Required:
- targeted map-related tests
- full suite

- [ ] Re-run the focused `agentkit map` validation slice
...
- annotated `v1.3.0` tag on origin

- [ ] Push the release branch or current release commit to origin
- [ ] Create or verify the annotated `v1.3.0` tag at the tested release commit
...
Required:
- `dist/` artifacts
- PyPI JSON/page proof for `agentkit-cli==1.3.0`

- [ ] Build wheel and sdist from the tested release state
- [ ] Publish `agentkit-cli==1.3.0` via the supported authenticated path
...
- [ ] Focused map-related validation slice passes
...
- [ ] Registry verification confirms both wheel and sdist for `1.3.0`
...
- Before trusting any release/status narrative, run:
  - `bash /Users/mordecai/.openclaw/workspace/scripts/pre-action-recall.sh release agentkit-cli /Users/mordecai/repos/agentkit-cli-v1.3.0-map`
